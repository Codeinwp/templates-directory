...
var obfx_template_directory = function ($) {
	...
	$(function () {
		...
		// Handle import click.
		$('.wp-full-overlay-header').on( 'click', '.obfx-import-template', function ( ev ) {
			...
			if (isDemoData ) {
				var event = new Event('importDemoData');
		...
		$('.obfx-preview-template').on('click', function () {
			...
			if ( isDemoData ) {
				$(document).trigger('initDemoImporter', {
					name: templateSlug,
					url: previewUrl
				});
			}
		...
		function setupImportButton() {
			...
			$(button).attr('data-isdemodata', $(activeTheme).data('isdemodata'));
